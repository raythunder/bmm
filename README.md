...
- [x] 登录用户可在首页卡片一键设置站点公开/取消公开，公开后同步展示在公共首页及对应标签页
...
- 在系统配置中可控制“是否开放注册”；关闭后登录页会隐藏注册入口，服务端也会拒绝注册请求。生产环境下会同时隐藏“Github 授权登录（免注册）”按钮；开发环境会始终显示该按钮，便于本地调试授权流程
...
- 左侧标签支持收藏/取消收藏；已收藏标签会显示在“快速添加站点”下方，可直接点击筛选
- 已收藏标签支持长按拖拽排序（收藏顺序按用户维度后端持久化，可跨设备同步）
- 用户关闭站点公开后，未登录访客不再可见；登录后在公共导航中仍可见并可再次开启公开
